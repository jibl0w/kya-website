...
export default function KYCPage() {
  ...
  async function handleSubmit() {
    ...
    // Saved. Move to consent, then Dojah identity verification (with liveness).
  ...
  // CRITICAL: user_id ties the webhook result back to THIS customer's profile.
  ...
  const dojahResponse = (type: string, data: unknown) => {
    ...
    if (type === "success" || type === "close") {
      // The authoritative result is written server-side by our webhook.
  ...
  // --- VERIFY STAGE: render the Dojah widget full-screen ---
  if (stage === "verify") {
    return (
      <Dojah
